...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[C]{\small DRAFT --- for internal review, not for distribution}
\fancyfoot[C]{\thepage}
...
\author{Micah Stubbs \and Yvonne Chen}
...
\begin{abstract}
Recent population-scale social simulators ground large language model (LLM)
agents in human data at extraordinary scale: Light Society simulates over one
billion agents grounded in World Values Survey demographics, and MatrAIx
maintains 8.3 billion persona records evaluated for behavioral adherence. Both
require infrastructure inaccessible to most researchers. We ask how much of
their methodology survives translation to a single commodity machine and a
public dataset. Using Sentiment140 --- 1.6M labeled tweets from 659{,}775 users,
occupying under 300\,MB in memory --- we reproduce three elements at small
scale: (i) human-grounded persona mining for the 6{,}245 users with at least 20
tweets; (ii) a mixture-of-models scale ladder that assigns full LLM agents,
distilled surrogates, or statistical samplers by per-user signal, cutting
simulated-population cost by $702.7\times$ versus an all-LLM population; and
(iii) ground-truth validation experiments on the real @-mention graph, where
DeGroot opinion diffusion ($r=0.54$) \emph{underperforms} simple per-user
persistence ($r=0.73$) in predicting future sentiment --- disposition beats
diffusion at this horizon. We further characterize relationship structure,
finding that sustained one-way (parasocial) attachments are as numerous as
strong mutual ties and that 12.1\% of mention volume flows to
broadcast/parasocial accounts, motivating a near-zero-cost broadcast tier in
population simulators. Finally, we contribute a fixed behavioral-adherence
benchmark for persona agents on this existing public dataset: a per-user
temporal 80/20 split over the cohort with sentiment adherence measured on
42{,}998 held-out tweets. A zero-parameter persona baseline attains 63.1\%
(chance is 50\%); MatrAIx's reported 91.5\% adherence, though measured under a
different protocol, frames the open gap. All code, data artifacts, and an
interactive explainer are public.
\end{abstract}
...
diurnal rhythm (Figure~\ref{fig:diurnal}) --- peak 23:00, trough 13:00 PDT ---
usable for calibrating agent posting schedules. Selection among candidate
...
which is precisely why persona grounding (\S\ref{sec:personas}) matters.
...
Strikingly, sustained one-way attachments (\emph{unidirectional strong}:
...
MatrAIx's headline metric --- do persona agents behave like their personas? ---
...
\textbf{Baseline.} The zero-parameter persona --- predict each user's
...
measures declared-attribute expression in synthetic interactions, ours
measures real held-out behavior --- so we frame 91.5\% as an aspirational bar
on our protocol, not a record to break on equal terms. The open question the
benchmark poses: how far above 63.1\% can persona-conditioned models go on
...
Night (August 2026). We thank Jake Schwartz for collaboration during the build
